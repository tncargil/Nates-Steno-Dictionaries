    ...
    "systems": [
        {
            ...
            "mods": {
                ...
                # "-FB": "",
                # "-PB": "",
                "-F": "control(c) v <middles> <objects>",  # Fisualise
                "-P": "<escape> y <middles> <objects> a",  # coPy
                "-B": "<escape> c <middles> <objects>",  # Blot
                "": "<escape> <middles> <objects> a",
...
relative_number_lookup = RelativeNumberLookup({
    ...
    "systems": {
        ...
        "W_right": {
            "stroke": "-BG",
            "callback": lambda x: f"{{#control(c) control(j) {addWhitespace(x)} w a}}",
            "dependencies": ["reverse", "double"],
            "min_number": 1,
            "max_number": 99,
            },
        "B_left": {
            "stroke": "-RG",
            "callback": lambda x: f"{{#control(c) control(j) {addWhitespace(x)} b a}}",
            "dependencies": ["reverse", "double"],
            "min_number": 1,
            "max_number": 99,
            },
        ...
        "up_remove": {
                "stroke": "R-B",
                "callback": lambda x: f"{{#control(c) control(j) d {addWhitespace(x)} k a}}",
                "dependencies": ["reverse", "double"],
                "min_number": 1,
                "max_number": 99,
                },
        "down_remove": {
                "stroke": "R-R",
                "callback": lambda x: f"{{#control(c) control(j) d {addWhitespace(x)} j a}}",
                "dependencies": ["reverse", "double"],
                "min_number": 1,
                "max_number": 99,
                }
    ...
    "systems": [
        {
            ...
            "mods": {  # 64 total possibilities from #EURPB
                ...
                "-F": "<escape>  control(<objects>) a",  # the good Spot (primeagen youtube video on the g command)
